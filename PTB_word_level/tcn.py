...
def gumsharp_softmax(logits,num_filters,seq_len, temperature, hard=False):

    y = sharp_softmax(logits, gamma = 8)
...
def sparsify(mask, seq_len, num_filters, level):
    ...
    w = mask.view(*shape)
    w = gumsharp_softmax(w,num_filters,seq_len, 0.1, True)
    w = torch.reshape(w, (num_filters, seq_len))
    return w


torch.set_printoptions(2, edgeitems=16 ,sci_mode=False)
...
class CausalConv1d(torch.nn.Conv1d):
    ...
    def forward(self, input):
        ...
        #print('padding is ',self.__padding)
...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, level, mask, gaw_1, gaw_2, skip_mask, stride, dilation, dropout=0.2):   ###ADATCN
    ...
    def forward(self, x):
        #first block
        y = torch.mul(x, sparsify(self.mask[0], self.mask.shape[-1], self.nout, self.level)) if self.level > 0 else self.identity1(x) 
        y = self.conv1(y)
        #y = self.chomp1(y) #needed for baseline tcn
        y = y if self.downsample is None else self.downsample(y)
        #Sparsify: for the first level / block, all elements will pass so we don't multiply with weights
        ...
        #return self.relu(out + res)#multiplying each block with its respective skip connection weight
    ...
    def __init__(self, seq_len, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            self.layers += [TemporalBlock(in_channels, out_channels, dilation_size *kernel_size , i, self.mask[i - 1],
                            self.gaw_1[i], self.gaw_2[i], self.skip_mask[i][0], stride=1, dilation= 1,
                            dropout=dropout)]
...
def gumsharp_softmax(logits,num_filters,seq_len,  hard=False):
    ...
    y_hard = y_hard.view(*shape)
    # Set gradients w.r.t. y_hard gradients w.r.t. y
    ## Reply: isn't it enought when I defined the weights in TemporalConvNet block, I set required_grad=True? 
...
class SE_block(nn.Module):
    ...
    def forward(self,input):
        ...
        #print('squeezed size, ', squeeze_tensor.size())

        # channel excitation
        fc_out_1 = self.activ_1(self.fc1(squeeze_tensor))
        #print('fc_out_1 size, ',fc_out_1.size())
        fc_out_2 = self.activ_2(self.fc2(fc_out_1))
        #print('fc_out_2 size,', fc_out_2.size())
        #out = torch.tensordot(input, fc_out_2, dims=([0,1],[0,1]))
...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #2nd block
        #print('for level {} outputs of conv is:, '.format(self.level), y[0][0])
        y = self.conv2(y)
        y = torch.mul(y, sparsify(self.mask[1], self.mask.shape[-1], self.nout, self.level)) if self.level > 0 else self.identity2(y)
        #y = self.chomp2(y) #needed for baseline TCN
        y = y if self.downsample is None else self.downsample(y)
...
class TemporalConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        ##UNCOMMENT THE FOLLOWING IF YOU WOULD LIKE TO CHECK THE OUTPUT OF THE CONVOLUTION
        #print('output is ', y[0])
        return y
